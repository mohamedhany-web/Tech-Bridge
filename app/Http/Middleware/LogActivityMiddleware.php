...
class LogActivityMiddleware
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // تسجيل الأنشطة فقط للمستخدمين المسجلين (بدون إيقاف الاستجابة إذا فشل التسجيل)
        if (Auth::check()) {
            try {
                $this->logActivity($request, $response);
            } catch (\Throwable $e) {
                \Log::debug('LogActivityMiddleware: ' . $e->getMessage());
            }
        }
    ...
    /**
     * الحصول على معرف النموذج من الـ route (بدون لمس النماذج المحذوفة)
     */
