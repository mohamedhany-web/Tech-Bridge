...
class ExamController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // جلب الاختبارات
        $query = AdvancedExam::whereHas('advancedCourse', function($q) use ($instructor) {
        ...
        // إحصائيات
        $stats = [
            'total' => AdvancedExam::whereHas('advancedCourse', function($q) use ($instructor) {
                $q->where('instructor_id', $instructor->id);
            })->count(),
            'active' => AdvancedExam::whereHas('advancedCourse', function($q) use ($instructor) {
                $q->where('instructor_id', $instructor->id);
            })->where('is_active', true)->count(),
            'total_attempts' => ExamAttempt::whereHas('exam.advancedCourse', function($q) use ($instructor) {
                $q->where('instructor_id', $instructor->id);
            })->count(),
            'completed_attempts' => ExamAttempt::whereHas('exam.advancedCourse', function($q) use ($instructor) {
                $q->where('instructor_id', $instructor->id);
    ...
    public function show(AdvancedExam $exam)
    {
        ...
        // التحقق من أن الاختبار يخص كورس يدرسه هذا المدرب
        if ($exam->advancedCourse->instructor_id !== $instructor->id) {
    ...
    public function edit(AdvancedExam $exam)
    {
        ...
        $lessons = \App\Models\CourseLesson::where('advanced_course_id', $exam->advanced_course_id)
            ->orderBy('order')
            ->get();
        
        $exam->load('questions');
        
        // جلب الأسئلة المتاحة من بنك أسئلة المدرب فقط (لا يطلع على بيانات المدربين الآخرين)
        $availableQuestions = Question::where('is_active', true)
            ->whereHas('questionBank', function ($q) use ($instructor) {
