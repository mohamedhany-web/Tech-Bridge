...
class AcademicYearController extends Controller
{
    ...
    /**
     * عرض كورسات المسار التعليمي مباشرة (بدون مجموعات مهارية)
     */
    public function subjects(AcademicYear $academicYear)
    {
        $courses = AdvancedCourse::where('academic_year_id', $academicYear->id)
            ->where('is_active', true)
            ->withCount('lessons')
            ->orderBy('title')
            ->get();

        $courses = $courses->map(function (AdvancedCourse $course) {
        ...
        return view('student.academic-years.courses', [
