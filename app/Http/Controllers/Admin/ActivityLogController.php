...
class ActivityLogController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // استبعاد تسجيلات الدخول المتكررة (افتراضياً) - يمكن تفعيلها من الفلتر
        // يتم استبعاد جميع تسجيلات الدخول ما لم يطلب المستخدم عرضها صراحة
        if (!$request->filled('include_logins') || $request->include_logins != '1') {
            $query->where(function($q) {
                $q->where('action', '!=', 'login')
                  ->where('action', '!=', 'user_login')
                  ->where('action', '!=', 'user_logged_in')
                  ->where('action', 'NOT LIKE', '%login%')
                  ->where('action', 'NOT LIKE', '%logged_in%');
            });
        }
