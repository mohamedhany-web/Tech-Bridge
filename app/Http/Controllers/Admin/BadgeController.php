...
class BadgeController extends Controller
{
    ...
    /** قيم type المسموحة في جدول badges (ENUM) */
    private const BADGE_TYPES = ['skill', 'milestone', 'special', 'seasonal'];

    private function mapCategoryToType(string $category): string
    {
        return in_array($category, self::BADGE_TYPES, true)
            ? $category
            : match ($category) {
                'excellence' => 'special',
                'dedication' => 'milestone',
                'leadership', 'community', 'other' => 'special',
                default => 'skill',
            };
    }
