...
class StudentEnrollmentController extends Controller
{
    ...
    {
        try {
            $query = StudentCourseEnrollment::with(['student', 'course' => fn ($q) => $q->with(['academicYear', 'academicSubject']), 'activatedBy']);
            ...
            return redirect()->route('admin.dashboard')->with('error', 'حدث خطأ في تحميل صفحة التسجيلات. راجع السجلات.');
        }
    }
    ...
    {
        $studentName = optional($enrollment->student)->name ?? 'طالب';
        $courseName = optional($enrollment->course)->title ?? 'كورس محذوف';
