...
class QuestionCategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'academic_subject_id.exists' => 'المادة الدراسية غير صالحة',
        ]);

        $data = $request->all();
        $data['is_active'] = $request->has('is_active');
        $data['academic_subject_id'] = $request->filled('academic_subject_id') ? $request->academic_subject_id : null;
        if (!isset($data['order'])) {
            $data['order'] = QuestionCategory::where('parent_id', $data['parent_id'] ?? null)->max('order') + 1;
        }
