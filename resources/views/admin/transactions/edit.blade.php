...
<div class="space-y-6">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">تعديل المعاملة</h1>
        ...
        <form action="{{ route('admin.transactions.update', $transaction) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" required class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 dark:bg-gray-700 dark:text-white">
                        <option value="credit" {{ $transaction->type == 'credit' ? 'selected' : '' }}>إيراد (دائن) - المال الذي دفعته لك</option>
                        <option value="debit" {{ $transaction->type == 'debit' ? 'selected' : '' }}>مصروف (مدين) - المال الذي دفعته أنت</option>
                    </select>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">دائن = إيراد (المال الذي دفعته لك)، مدين = مصروف (المال الذي دفعته أنت)</p>
                </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 dark:bg-gray-700 dark:text-white">{{ old('description', $transaction->description) }}</textarea>
            </div>
            ...
            <div class="flex gap-4">
                ...
                <a href="{{ route('admin.transactions.index') }}" class="bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-800 dark:text-white px-6 py-3 rounded-lg font-medium transition-colors">
                    إلغاء
                </a>
            </div>
        </form>
