        ...
        <div class="rounded-2xl bg-white dark:bg-gray-900 shadow-lg border border-sky-100 dark:border-gray-700 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-black text-gray-900 dark:text-white">{{ number_format($plan->total_amount ?? 0, 2) }} ج.م</p>
                </div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-3">القيمة الكاملة للخطة قبل الدفعات المقدمة.</p>
        </div>
        <div class="rounded-2xl bg-white dark:bg-gray-900 shadow-lg border border-amber-100 dark:border-gray-700 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-black text-gray-900 dark:text-white">{{ number_format($plan->deposit_amount ?? 0, 2) }} ج.م</p>
                </div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-3">المبلغ المطلوب دفعه مقدماً قبل بدء التقسيط.</p>
        </div>
        <div class="rounded-2xl bg-white dark:bg-gray-900 shadow-lg border border-emerald-100 dark:border-gray-700 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-black text-gray-900 dark:text-white">{{ number_format($agreementsCount) }}</p>
                </div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-3">{{ number_format($activeAgreements) }} اتفاقيات نشطة مرتبطة بالخطة.</p>
        </div>
        <div class="rounded-2xl bg-white dark:bg-gray-900 shadow-lg border border-purple-100 dark:border-gray-700 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-black text-gray-900 dark:text-white">{{ number_format($averageInstallments, 1) }}</p>
                </div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-3">متوسط عدد الدفعات للاتفاقيات المرتبطة.</p>
        </div>
    </div>

    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div class="xl:col-span-2 space-y-6">
            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 space-y-6">
                <h2 class="text-lg font-black text-gray-900 dark:text-white">تفاصيل الخطة</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">الكورس المرتبط</p>
                        <p class="mt-2 text-base font-semibold text-gray-900 dark:text-white">{{ $plan->course->title ?? 'خطة عامة' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">القيمة الممولة</p>
                        <p class="mt-2 text-base font-semibold text-gray-900 dark:text-white">{{ number_format($totalFinanced, 2) }} ج.م</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">إجمالي الدفعات المقدمة</p>
                        <p class="mt-2 text-base font-semibold text-gray-900 dark:text-white">{{ number_format($totalDeposits, 2) }} ج.م</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">ملاحظات إضافية</p>
                        <p class="mt-2 text-base font-semibold text-gray-900 dark:text-white">{{ data_get($plan->metadata, 'notes', '—') }}</p>
                    </div>
                </div>
                <div class="bg-gray-50 dark:bg-gray-800/60 rounded-2xl px-4 py-3 text-xs text-gray-500 dark:text-gray-400">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-lg font-black text-gray-900 dark:text-white">الاتفاقيات المرتبطة</h2>
                ...
                @if($agreementsCount > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($agreements as $agreement)
                            <div class="p-4 rounded-2xl border border-gray-100 dark:border-gray-700 bg-gray-50/70 dark:bg-gray-800/60 space-y-3">
                                <div class="flex items-center justify-between">
                                    <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ $agreement->user->name ?? 'مستخدم غير معروف' }}</p>
                                    <span class="text-xs text-gray-500 dark:text-gray-400">{{ optional($agreement->created_at)->diffForHumans() }}</span>
                                </div>
                                <p class="text-xs text-sky-600 dark:text-sky-300">{{ $agreement->course->title ?? 'خطة عامة' }}</p>
                                <div class="grid grid-cols-2 gap-3 text-xs text-gray-600 dark:text-gray-300">
                                    <div>
                                        <p class="uppercase text-[11px]">إجمالي الاتفاقية</p>
                                        <p class="mt-1 text-sm font-semibold text-gray-900 dark:text-white">{{ number_format($agreement->total_amount ?? 0, 2) }} ج.م</p>
                                    </div>
                                    <div>
                                        <p class="uppercase text-[11px]">دفعة مقدمة</p>
                                        <p class="mt-1 text-sm font-semibold text-gray-900 dark:text-white">{{ number_format($agreement->deposit_amount ?? 0, 2) }} ج.م</p>
                                    </div>
                                    <div>
                                        <p class="uppercase text-[11px]">عدد الأقساط</p>
                                        <p class="mt-1 text-sm font-semibold text-gray-900 dark:text-white">{{ $agreement->installments_count }} دفعة</p>
                                    </div>
                                    <div>
                                        <p class="uppercase text-[11px]">الحالة</p>
                                        <p class="mt-1 text-sm font-semibold text-gray-900 dark:text-white">{{ $agreement->status }}</p>
                                    </div>
                                </div>
                                @if($agreement->notes)
                                    <p class="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">{{ $agreement->notes }}</p>
                                @endif
                            </div>
                        @endforeach
                    </div>
                @else
                    <div class="text-sm text-gray-500 dark:text-gray-400 text-center py-10">
        ...
        <div class="space-y-6">
            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
                <h2 class="text-lg font-black text-gray-900 dark:text-white mb-4">مخطط الدفعات</h2>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
                ...
                <div class="space-y-3">
                    <a href="{{ route('admin.installments.plans.edit', $plan) }}" class="flex items-center justify-between px-4 py-3 rounded-2xl border border-sky-100 dark:border-sky-900/40 bg-sky-50/70 dark:bg-sky-900/20 text-sky-600 dark:text-sky-300 hover:border-sky-200 dark:hover:border-sky-700 transition-all">
                    ...
                    <a href="{{ route('admin.installments.plans.index') }}" class="flex items-center justify-between px-4 py-3 rounded-2xl border border-gray-100 dark:border-gray-800 bg-gray-50/70 dark:bg-gray-800/20 text-gray-600 dark:text-gray-300 hover:border-gray-200 dark:hover:border-gray-700 transition-all">
                        <div class="flex items-center_gap-3">
                            <span class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300">
