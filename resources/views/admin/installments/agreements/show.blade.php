...
<div class="container mx-auto px-4 py-8 space-y-8">
    <div class="bg-gradient-to-br from-emerald-500 via-emerald-600 to-sky-600 rounded-3xl shadow-xl text-white p-8 relative overflow-hidden">
        ...
        <div class="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div class="flex flex-wrap gap-3 justify-end">
                <a href="{{ route('admin.installments.agreements.edit', $agreement) }}" class="inline-flex items-center_gap-2 px-5 py-3 rounded-2xl bg-white text-emerald-700 font-semibold shadow-lg hover:shadow-xl transition-all">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <div class="rounded-2xl bg-white dark:bg-gray-900 shadow-lg border border-sky-100 dark:border-gray-700 p-6">
            ...
            <p class="mt-2 text-3xl font-black text-gray-900 dark:text-white">{{ number_format($agreement->total_amount ?? 0, 2) }} ج.م</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-3">القيمة الكاملة التي سيتم سدادها عبر الخطة.</p>
        </div>
        <div class="rounded-2xl bg-white dark:bg-gray-900 shadow-lg border border-emerald-100 dark:border-gray-700 p-6">
            ...
            <p class="mt-2 text-3xl font-black text-gray-900 dark:text-white">{{ number_format($agreement->deposit_amount ?? 0, 2) }} ج.م</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-3">تم تحصيلها عند توقيع الاتفاقية.</p>
        </div>
        <div class="rounded-2xl bg-white dark:bg-gray-900 shadow-lg border border-amber-100.dark:border-gray-700 p-6">
            ...
            <p class="mt-2 text-3xl font-black text-gray-900 dark:text-white">{{ $pendingPayments->count() }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-3">القيمة التالية: {{ optional($nextPayment)->amount ? number_format($nextPayment->amount, 2) . ' ج.م' : '—' }}</p>
        </div>
        <div class="rounded-2xl bg-white dark:bg-gray-900 shadow-lg border border-purple-100 dark:border-gray-700 p-6">
            ...
            <p class="mt-2 text-3xl font-black text-gray-900 dark:text-white">{{ optional($nextPayment)->due_date?->format('Y-m-d') ?? '—' }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-3">عدد الأقساط الكلي: {{ $agreement->installments_count }}</p>
        </div>
    </div>

    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div class="xl:col-span-2 space-y-6">
            <div class="bg-white.dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800 p-6 space-y-6">
                <h2 class="text-lg font-black text-gray-900 dark:text-white">تفاصيل الطالب والكورس</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">اسم الطالب</p>
                        <p class="mt-2 text-base font-semibold text-gray-900 dark:text-white">{{ $student->name ?? 'غير متوفر' }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ $student->phone ?? 'بدون هاتف' }} · {{ $student->email ?? 'بدون بريد' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">الكورس</p>
                        <p class="mt-2 text-base font-semibold text-gray-900 dark:text-white">{{ $course->title ?? 'خطة عامة' }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">سعر الكورس: {{ number_format($course->price ?? 0, 2) }} ج.م</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">الخطة</p>
                        <p class="mt-2 text-base font-semibold text-gray-900 dark:text-white">{{ $plan->name ?? 'غير محددة' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500.dark:text-gray-400 uppercase">دورية الأقساط</p>
                        <p class="mt-2 text-base font-semibold text-gray-900 dark:text-white">كل {{ $plan->frequency_interval ?? '—' }} {{ ($plan && $frequencyUnits[$plan->frequency_unit] ?? $plan->frequency_unit ?? '-') }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">فترة السماح: {{ $plan->grace_period_days ?? 0 }} يوم</p>
                    </div>
                </div>
                @if($agreement->notes)
                    <div class="bg-gray-50 dark:bg-gray-800/60 rounded-2xl px-4 py-3 text-xs text-gray-600 dark:text-gray-300 leading-relaxed">
                        <strong class="text-sm text-gray-900 dark:text-white">ملاحظات الاتفاقية:</strong>
                        <p class="mt-2">{{ $agreement->notes }}</p>
                    </div>
                @endif
            </div>

            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800 p-6 space-y-4">
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-black text-gray-900 dark:text-white">جدول الأقساط</h2>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm text-gray-700 dark:text-gray-200 divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($payments as $payment)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/60 transition-colors">
                                    ...
                                    <td class="px-4 py-3 font-semibold text-gray-900 dark:text-white">{{ number_format($payment->amount ?? 0, 2) }} ج.م</td>
                                    <td class="px-4 py-3">
                                        <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold
                                            @class([
                                    ...
                                    <td class="px-4 py-3 text-left">
                                        <form action="{{ route('admin.installments.payments.mark', $payment) }}" method="POST" class="inline-flex gap-2">
                                            @csrf
                                            ...
                                            <button type="submit" class="text-xs px-3 py-1 rounded-xl bg-sky-100 text-sky-600 hover:bg-sky-200 dark:bg-sky-900/40 dark:text-sky-300">
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-4 py-10 text-center text-gray-500 dark:text-gray-400 text-sm">لم يتم توليد جدول أقساط بعد.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="space-y-6">
            <div class="bg-white dark:bg-gray-900 rounded-3xl shadow-lg border border-gray-100 dark:border-gray-800 p-6">
                <h2 class="text-lg font-black text-gray-900 dark:text-white mb-4">نظرة سريعة</h2>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-3xl shadow-lg border border-gray-100 dark:border-gray-800 p-6">
                <h2 class="text-lg font-black text-gray-900 dark:text-white mb-4">إجراءات إضافية</h2>
                <div class="space-y-3">
                    <form action="{{ route('admin.installments.agreements.destroy', $agreement) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من إلغاء الاتفاقية؟');" class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="flex items-center justify-between w-full px-4 py-3 rounded-2xl border border-rose-100 dark:border-rose-900/40 bg-rose-50/70 dark:bg-rose-900/20 text-rose-600 dark:text-rose-300 hover:border-rose-200 dark:hover:border-rose-700 transition-all">
