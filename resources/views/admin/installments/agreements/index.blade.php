    ...
    <div class="bg-gradient-to-br from-sky-500 via-sky-600 to-purple-600 rounded-3xl shadow-xl text-white p-8 relative overflow-hidden">
        ...
        <div class="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div>
                ...
                <div class="mt-4 inline-flex items-center gap-2 rounded-xl bg-white/20 px-4 py-2 text-xs font-semibold text-white">
                    <i class="fas fa-info-circle"></i>
                    <span>اتفاقية السداد: اتفاقية فعلية بين طالب معين والمنصة بناءً على خطة تقسيط. كل طالب له اتفاقية خاصة به مرتبطة بخطة تقسيط.</span>
                </div>
            </div>
        ...
        <div class="rounded-2xl bg-white dark:bg-gray-900 shadow-lg border border-sky-100 dark:border-gray-700 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-black text-gray-900 dark:text-white">{{ number_format($agreements->where('status', \App\Models\InstallmentAgreement::STATUS_ACTIVE)->count()) }}</p>
                </div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-3">الانتقالات الحالية التي تتطلب متابعة دورية.</p>
        </div>
        <div class="rounded-2xl bg-white dark:bg-gray-900 shadow-lg border border-emerald-100 dark:border-gray-700 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-black text-gray-900 dark:text-white">{{ number_format($agreements->sum('total_amount'), 2) }} ج.م</p>
                </div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-3">القيمة الإجمالية التي تغطيها جميع الاتفاقيات.</p>
        </div>
        <div class="rounded-2xl bg-white dark:bg-gray-900 shadow-lg border border-amber-100 dark:border-gray-700 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl.font-black text-gray-900 dark:text-white">{{ number_format($agreements->sum('deposit_amount'), 2) }} ج.م</p>
                </div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-3">إجمالي المبالغ المحصلة كدفعات مقدمة.</p>
        </div>
        <div class="rounded-2xl bg-white dark:bg-gray-900 shadow-lg border border-purple-100 dark:border-gray-700 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-black text-gray-900 dark:text-white">{{ number_format($agreements->where('status', \App\Models\InstallmentAgreement::STATUS_OVERDUE)->count()) }}</p>
                </div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-3">الاتفاقيات التي تحتاج تدخلاً بسبب تأخر السداد.</p>
        </div>
    </div>

    <div class="bg-white dark:bg-gray-900 rounded-3xl shadow-lg border border-gray-100 dark:border-gray-800 p-8 space-y-6">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-gray-900 dark:text-white">جميع الاتفاقيات</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">تتبع حالة الطلاب، المبالغ المتبقية، وجداول السداد.</p>
            </div>
            <form method="GET" class="flex flex-wrap gap-3 items-center">
                <select name="status" class="px-4 py-2 rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm text-gray-700 dark:text-gray-200">
        ...
        @if($agreements->count())
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                @foreach($agreements as $agreement)
                    <div class="rounded-3xl border border-gray-100 dark:border-gray-800 bg-gray-50/70 dark:bg-gray-900/60 p-6 flex flex-col gap-4">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ $agreement->student->name ?? 'طالب غير معروف' }}</p>
                                <p class="text-xs text-sky-600 dark:text-sky-300 mt-1">{{ $agreement->course->title ?? 'خطة عامة' }}</p>
                                <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-1">بداية {{ optional($agreement->start_date)->format('Y-m-d') }}</p>
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">إجمالي الاتفاقية</p>
                                <p class="mt-1 text-base font-semibold text-gray-900 dark:text-white">{{ number_format($agreement->total_amount ?? 0, 2) }} ج.م</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">دفعة مقدمة</p>
                                <p class="mt-1 text-base font-semibold text-gray-900 dark:text-white">{{ number_format($agreement->deposit_amount ?? 0, 2) }} ج.م</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">عدد الأقساط</p>
                                <p class="mt-1 text-base font-semibold text-gray-900 dark:text-white">{{ $agreement->installments_count }} دفعة</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">القسط التالي</p>
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            <a href="{{ route('admin.installments.agreements.show', $agreement) }}" class="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2 rounded-2xl bg-sky-100 text-sky-600 dark:bg-sky-900/40 dark:text-sky-300 font-semibold hover:bg-sky-200 dark:hover:bg-sky-900/60 transition-all">
                                <i class="fas fa-eye"></i>
                                عرض التفاصيل
                            </a>
                            <a href="{{ route('admin.installments.agreements.edit', $agreement) }}" class="inline-flex items-center justify-center w-10 h-10 rounded-2xl bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 transition-all" title="تعديل">
