...
<div class="px-4 py-8">
    <div class="max-w-5xl mx-auto space-y-8">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-3xl shadow-xl border border-gray-100/60 dark:border-gray-800 overflow-hidden" x-data="{ walletType: '{{ old('type', $wallet->type) }}' }">
            <div class="border-b border-gray-100 dark:border-gray-800 px-6 sm:px-8 py-5 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div>
                    <h2 class="text-xl font-semibold text-gray-900 dark:text-white">بيانات المحفظة</h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400">قم بتحديث معلومات المحفظة واحفظ التغييرات.</p>
                </div>
                <a href="{{ route('admin.wallets.show', $wallet) }}" class="inline-flex items-center gap-2 text-sm font-medium text-sky-600 dark:text-sky-400 hover:text-sky-700">
            ...
            <form action="{{ route('admin.wallets.update', $wallet) }}" method="POST" class="p-6 sm:p-8 space-y-10">
                ...
                <section class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">اسم المحفظة *</label>
                            <input type="text" name="name" value="{{ old('name', $wallet->name) }}" required
                                   class="w-full rounded-2xl border border-gray-200 dark:border-gray-700 bg-white/60 dark:bg-gray-900/70 px-4 py-3 text-gray-900 dark:text-gray-100 shadow-sm focus:border-sky-500 focus:ring-4 focus:ring-sky-500/20 transition">
                            @error('name')
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">نوع المحفظة *</label>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">رقم الحساب/المحفظة</label>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">اسم صاحب الحساب</label>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">الرصيد الحالي</label>
                            <div class="relative">
                                <input type="number" name="balance" value="{{ old('balance', $wallet->balance) }}" step="0.01" min="0" readonly
                                       class="w-full rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 px-4 py-3 text-gray-900 dark:text-gray-100 shadow-sm cursor-not-allowed">
                ...
                <section class="space-y-6">
                    <div>
                        ...
                        <textarea name="notes" rows="4"
                                  class="w-full rounded-2xl border border-gray-200 dark:border-gray-700 bg-white/60 dark:bg-gray-900/70 px-4 py-3 text-gray-900 dark:text-gray-100 shadow-sm focus:border-sky-500 focus:ring-4 focus:ring-sky-500/20 transition">{{ old('notes', $wallet->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-2 text-sm text-rose-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center gap-3 px-4 py-3 rounded-2xl bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-gray-800 dark:text-gray-200">تفعيل المحفظة</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400">فعّل هذا الخيار للسماح باستخدام المحفظة.</p>
                        </div>
                    </div>
                </section>

                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                    <a href="{{ route('admin.wallets.index') }}" class="inline-flex items-center justify-center gap-2 rounded-2xl border border-gray-200 dark:border-gray-700 px-5 py-3 text-sm font-semibold text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                        <i class="fas fa-arrow-right ml-1"></i>
                        إلغاء
                    </a>
