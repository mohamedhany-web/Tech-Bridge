...
@section('title', 'تعديل الملف - Tech Bridge')
...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    <div class="bg-white dark:bg-gray-800 shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">تعديل الملف</h1>
                    <p class="mt-2 text-gray-600 dark:text-gray-400">{{ $media->title }}</p>
                </div>
                <div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form action="{{ route('admin.media.update', $media) }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white">{{ old('description', $media->description) }}</textarea>
                    @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                @if($media->file_path)
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">الملف الحالي</label>
                    @if($media->type === 'image')
                    <img src="{{ asset($media->file_path) }}" alt="{{ $media->title }}" class="h-32 w-32 object-cover rounded-lg mb-2">
                    @else
                    <p class="text-sm text-gray-600 dark:text-gray-400">فيديو: {{ $media->file_path }}</p>
                    @endif
                ...
                <!-- الملف الجديد -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">الملف الجديد (اختياري)</label>
                ...
                <div id="thumbnail-section" style="display: {{ old('type', $media->type) == 'video' ? 'block' : 'none' }};">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">الصورة المصغرة</label>
                ...
                <div class="flex items-center">
                    ...
                    <label class="mr-2 text-sm font-medium text-gray-700 dark:text-gray-300">مميز</label>
                </div>
