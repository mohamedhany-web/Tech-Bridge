...
<div class="space-y-6">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
        <div class="flex justify-between items-start mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $achievement->name }}</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-1">{{ $achievement->category ?? $achievement->type ?? '-' }}</p>
            </div>
            <div>
                ...
                <a href="{{ route('admin.achievements.index') }}" class="bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-800 dark:text-white px-4 py-2 rounded-lg font-medium transition-colors">
                    رجوع
                </a>
            </div>
        </div>
            ...
            <div>
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <p class="font-medium text-gray-900 dark:text-white mt-1">{{ $achievement->description }}</p>
                    </div>
                    @endif
                    <div>
                        <span class="text-sm text-gray-600 dark:text-gray-400">الفئة:</span>
                        <span class="font-medium text-gray-900 dark:text-white mr-2">{{ $achievement->category ?? $achievement->type ?? '-' }}</span>
                    </div>
                    @if($achievement->points_reward ?? $achievement->points)
                    <div>
                        <span class="text-sm text-gray-600 dark:text-gray-400">النقاط:</span>
            ...
            <div>
                ...
                <div class="space-y-3">
                    <div class="bg-gradient-to-br from-sky-50 to-blue-50 dark:from-sky-900/20 dark:to-blue-900/20 rounded-xl p-4 border border-sky-200 dark:border-sky-800">
                        ...
                        <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ $achievement->users_count ?? $achievement->users->count() ?? 0 }}</div>
                    </div>
        ...
        <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
            <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">المستخدمون الحاصلون على هذا الإنجاز</h3>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-4">و {{ $achievement->users->count() - 12 }} مستخدم آخر</p>
            @endif
        </div>
        @endif
    </div>
