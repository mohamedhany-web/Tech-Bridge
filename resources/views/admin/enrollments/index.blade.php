...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-shadow duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 dark:text-gray-400">معلق</p>
                    <p class="text-3xl font-bold text-gray-900 dark:text-white">{{ number_format($stats['suspended'] ?? 0) }}</p>
                </div>
            ...
            <div class="mt-4">
                <span class="text-sm text-red-600 dark:text-red-400">إيقاف تسجيل — يمكن إعادة التفعيل</span>
            </div>
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900 dark:text-white">{{ optional($enrollment->course)->title ?? 'كورس محذوف' }}</div>
                                <div class="text-sm text-gray-500 dark:text-gray-400">
                                    {{ optional($enrollment->course)->academicYear?->name ?? 'غير محدد' }} — 
                                    {{ optional($enrollment->course)->academicSubject?->name ?? 'غير محدد' }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                {{ optional($enrollment->enrolled_at)->format('d/m/Y') ?? '—' }}
                            </td>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    @elseif($enrollment->status === 'active')
                                        <form method="POST" action="{{ route('admin.enrollments.deactivate', $enrollment) }}" class="inline">
                                            @csrf
                                            <button type="submit" class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300 hover:bg-orange-200 dark:hover:bg-orange-800/50" 
                                    ...
                                    @elseif($enrollment->status === 'suspended')
                                        <form method="POST" action="{{ route('admin.enrollments.activate', $enrollment) }}" class="inline">
                                            @csrf
                                            <button type="submit" class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300 hover:bg-emerald-200 dark:hover:bg-emerald-800/50" 
