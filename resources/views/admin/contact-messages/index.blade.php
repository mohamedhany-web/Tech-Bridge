...
@section('title', 'إدارة رسائل التواصل - Tech Bridge')
...
<div class="p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
    ...
    <div class="mb-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div>
                ...
                <p class="text-gray-600 dark:text-gray-400">{{ __('عرض وإدارة رسائل التواصل من الزوار') }}</p>
                @if($stats['unread'] > 0)
                <div class="mt-3 inline-flex items-center gap-2 px-4 py-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 p-6 border-r-4 border-red-500">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    <p class="text-gray-500 dark:text-gray-400 text-sm font-medium mb-1">{{ __('غير مقروءة') }}</p>
                    <p class="text-3xl font-bold text-gray-900 dark:text-white">{{ number_format($stats['unread']) }}</p>
                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-2">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 p-6 border-r-4 border-emerald-500">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    <p class="text-gray-500 dark:text-gray-400 text-sm font-medium mb-1">{{ __('مقروءة') }}</p>
                    <p class="text-3xl font-bold text-gray-900 dark:text-white">{{ number_format($stats['read']) }}</p>
                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-2">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach($messages as $message)
                            <tr class="hover:bg-gradient-to-r hover:from-sky-50 hover:to-blue-50 dark:hover:from-gray-700 dark:hover:to-gray-700 transition-all duration-200 {{ $message->status === 'new' ? 'bg-sky-50/50 dark:bg-sky-900/10' : '' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold shadow-sm bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-200">
                                        <i class="fas fa-check-circle"></i>
                                        <span>مقروءة</span>
                                    </span>
                                    @else
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold shadow-sm bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('admin.contact-messages.mark-as-unread', $message) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" 
                                                    class="w-9 h-9 flex items-center justify-center bg-amber-50 hover:bg-amber-100 dark:bg-amber-900 dark:hover:bg-amber-800 text-amber-600 dark:text-amber-300 rounded-lg transition-colors"
