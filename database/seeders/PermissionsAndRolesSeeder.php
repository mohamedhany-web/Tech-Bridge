...
class PermissionsAndRolesSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $permissions = [
            ...
            // إدارة المحتوى (CMS)
            ['name' => 'blog.view', 'display_name' => 'عرض المدونة', 'group' => 'cms'],
            ['name' => 'blog.create', 'display_name' => 'إنشاء مقالات', 'group' => 'cms'],
            ['name' => 'blog.edit', 'display_name' => 'تعديل المقالات', 'group' => 'cms'],
            ['name' => 'blog.delete', 'display_name' => 'حذف المقالات', 'group' => 'cms'],
        ...
        // صلاحيات الطالب (المستخدمة في القائمة الجانبية ولوحة التحكم)
        ...
        $studentPermissions = Permission::whereIn('name', $studentPermissionNames)->pluck('id');
        if ($studentPermissions->isNotEmpty()) {
            $studentRole->permissions()->sync($studentPermissions);
        } else {
            // توافق مع أسماء الصلاحيات القديمة إن وُجدت فقط
            $studentRole->permissions()->sync(
                Permission::whereIn('name', [
                    'courses.view', 'lectures.view', 'assignments.view',
                    'exams.view', 'exams.take', 'tasks.view', 'tasks.create', 'tasks.edit',
                    'invoices.view', 'certificates.view',
                ])->pluck('id')
            );
        }
